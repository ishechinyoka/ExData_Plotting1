...
baltimore_md_pm25 <- summary_pm25 %>% filter(fips == "24510") %>% group_by(year) %>% count(Emissions)


png("plot2.png")

barplot(baltimore_md_pm25$Emissions, names = baltimore_md_pm25$year, xlab = "Years", ylab = "Emissions", main = "Emissions in Baltimore City")
